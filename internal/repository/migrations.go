...
func InitDB(db DBTX, dbType string) error {
	...
	CREATE TABLE IF NOT EXISTS hosts (id SERIAL PRIMARY KEY, user_id INTEGER REFERENCES users(id), name TEXT NOT NULL, address TEXT NOT NULL, port INTEGER DEFAULT 22, username TEXT NOT NULL, key_id INTEGER REFERENCES keys(id), created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP);
	`
    ...
    CREATE TABLE IF NOT EXISTS hosts (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER REFERENCES users(id), name TEXT NOT NULL, address TEXT NOT NULL, port INTEGER DEFAULT 22, username TEXT NOT NULL, key_id INTEGER REFERENCES keys(id), created_at DATETIME DEFAULT CURRENT_TIMESTAMP);
	`
